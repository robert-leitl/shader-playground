import {
  ...
  ElementRef, AfterViewInit, NgZone, OnDestroy
} from '@angular/core';
...
export class ImageTransition1Component implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    this.sketch = new ImageTransition1Sketch(this.hostElementRef.nativeElement);
    this.sketch.oninit = () => {
  ...
  onMouseOver(e: Event): void {
    ...
    if (linkElm && linkElm.parentElement) {
      ...
      for(; i<linkElm.parentElement.children.length; ++i) {
        if (linkElm.parentElement.children[i] === linkElm)
          break;
      }
